...
import { getModuleList } from './module'
 ...
 * MO FIXED 兼容其他 rc 文件
 */
export async function setAppRC(
  appPath: string,
  appRC: IAppRC | Partial<IAppRC>
) {
...
export function setAppRmxConfig(key, value, packageFilePath) {
  console.trace(
    '@deprecated setAppRmxConfig(key, value, pkgPath) => setAppRCPart(key, value, appPath)'
  )
