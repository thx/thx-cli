...
export function withSpinner (title: string, task: Function, errorHandler?: Function) {
  return async (emitter?: EventEmitter, ...args) => {
    ...
    try {
      ...
      emitter && emitter.emit('data', greenBright(`✔ ${title} ${Date.now() - start}ms`))
