...
export default function registerCommand(
  ...
  // 命令名称
  const subcommand = program.command(
    commandConfig.command || commandConfig.name
  )
  // 命令别名
  if (commandConfig.alias) subcommand.alias(commandConfig.alias)
  // 命令描述
  if (commandConfig.description)
    subcommand.description(commandConfig.description)
  // 命令参数
  ...
  subcommand.action(async (...argv) => {
    ...
    // 命令主函数
    const actionResult =
      commandConfig.action && (await commandConfig.action(...argv, params))

    // 命令后勾子
    if (after) await after(...argv, actionResult)

    logger.debug(
      `⌚️ The execution of command '${
