...
const util = {
   ...
   * @deprecated
   */
  getExistFile (fileName: string, cwd?: string): Promise<any> {
  ...
  // 获取项目的RAP的id
  async getRapProjectId (cwd: string = process.cwd()) {
    ...
    return rmxConfig.rapProjectId || magixCliConfig.rapProjectId || magixCliConfig.matProjectId
  },

  // 从项目本地package.json里获取magixCliConfig配置
  async getMagixCliConfig (cwd?) {
    const appPath = await utils.getAppPath(cwd)
    const pkg = await utils.getAppPkg(appPath)
    const magixCliConfig = pkg.magixCliConfig || {}
  ...
  // 如果本地有模板则用本地的模板
  async getTmplGenerater (originPath, configKey, cwd): Promise<any> {
    const magixCliConfig = await this.getMagixCliConfig(cwd)
    ...
    if (magixCliConfig[configKey]) {
      const isExistModelsTmpl = await this.getExistFile(magixCliConfig[configKey], cwd)
      if (isExistModelsTmpl) {
        const managerFile = fs.readFileSync(Path.resolve(isExistModelsTmpl.join('/'), magixCliConfig[configKey]), 'utf-8')
  ...
  // 拉取各平台接口生成本地配置文件
  async generateLocalConfig (options) {
    const magixCliConfig = options.magixCliConfig
    let resp
    try {
      resp = options.resp || await requestP({
        url: options.api,
    ...
    // 生成models.js的路径默认值
    // 优先获取package.json里配置
    const originConfigPath = magixCliConfig[options.configPathKey] || options.defaultConfigPath

    if (!originConfigPath) {
      throw new Error('缺失生成文件路径配置')
    }

    const tmplGenerater = await this.getTmplGenerater(options.defaultConfigTmplPath, options.configTmplKey, options.cwd)
  ...
  compatGalleriesConfig (magixCliConfig: any = {}) {
