  ...
  async function modifiedTips (modifieds, tips) {
    ...
    modifieds.forEach(mod => {
      mod.modifyFiles.forEach(file => {
        console.log(chalk.grey(' ├──'), chalk.cyan(`${file.filePath}${chalk.grey(' - ')}${file.type === 'deleted' ? chalk.red(`[${file.type}]`) : chalk.yellow(`[${file.type}]`)
                    }`))
      })
    })
    console.log()
  ...
  if (list) {
    try {
      ...
      for (const gallery of gallerys) {
        console.log(chalk.green(` 组件库[${gallery.repoName}]`), chalk.grey(`${gallery.path}`))
        for (const g of gallery.list) {
          console.log(chalk.grey(` ├── ${chalk.cyan(g.name)} ${chalk.grey(`${g.version}`)}`))
        }
      }
    } catch (error) {
    ...
    galleryApi
      ...
      .on('close', async resp => {
        ...
        if (modifiedGalleryArr.length) {
          isConfirm = await modifiedTips(modifiedGalleryArr, '确认要覆盖升级吗？(选择 no 则只升级本地未修改过的组件)')
        }

        galleryApi
          .exec({
            name: options.galleryName,
