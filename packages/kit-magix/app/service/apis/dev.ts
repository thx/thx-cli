...
export default {
  ...
  exec(params: any = {}) {
    ...
    setTimeout(async () => {
      ...
      if (magixCliConfig.devCommand) {
        devCommand = magixCliConfig.devCommand
      }

      if (magixCliConfig.proxyCommand) {
        proxyCommand = magixCliConfig.proxyCommand
      }
      ...
      if (magixCliConfig.magixDesigerTemplate && !params.isCloseDesiger) {
        try {
          const defaultDesignTemplate = magixCliConfig.magixDesigerTemplate
          // if (options.template && options.template !== true) {
          //     defaultDesignTemplate = options.template
          // }
          const appWrapper = require('@ali/magix-desiger-server/app')
