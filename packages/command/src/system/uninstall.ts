...
async function commandAction(
  ...
  if (!installedModuleList.length) {
    const message = yellowBright(
      `ⓘ 尚未安装${MODULE_TYPE_MAP[type]} ${name || ''}`
    )
    logger.warn(message)
    console.log(message)
    return
  ...
  if (name === undefined) {
    const installedModuleQuestionList = await prepareModuleQuestionList(
      type,
      installedModuleList
    )
    const installedModuleAnswers = await inquirer.prompt(
      installedModuleQuestionList
    )
    module = installedModuleAnswers.module
  } else {
    module = installedModuleList.find(
...
const commandConfig: ICommandConfig = {
  ...
  on: [
    [
      ...
      () => {
        console.log('\nExamples:')
        console.log(`  ${grey('$')} ${blueBright(`${CLI_NAME} uninstall`)}`)
      }
    ]
  ]
}
