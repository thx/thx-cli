...
function getPluginCommandModule (pluginInfo: IPluginInfo) {
  ...
  try {
    ...
    pluginCommandModule = pluginCommandModule.__esModule ? pluginCommandModule.default : pluginCommandModule
  } catch (error) {
    logger.error(error)
    // console.error(error)
  }
...
export default async function registerPluginCommand (program: CommanderStatic, pluginInfo: IPluginInfo) {
  logger.info('registerPluginCommand', pluginInfo)
  const { name: pluginName, package: packageName, command: { command, alias } } = pluginInfo
