...
// MO FIXED reduce 会导致类型推导错误
// const defaultKitCommandMap = defaultKitCommandList.reduce((acc, cur) => {
//   acc[cur.name || cur.command] = cur
//   return acc
// }, {})
...
function getKitCommandListModule (kitInfo: IKitInfo) {
  ...
  try {
    utils.took('getKitCommandListModule', () => {
      kitCommandListModule = require(`${RMX_HOME}/${type}/${kitName}/node_modules/${packageName}/dist/commands`)
    }, logger)
...
function isSystemCommand (kitCommandConfig: ICommandConfig): boolean {
  const kitCommandName = kitCommandConfig.name || kitCommandConfig.command
  const existed = !!systemCommandList.find(({ name, command, alias }) =>
...
export default async function registerKitCommandList (program: CommanderStatic, kitInfo: IKitInfo) {
  ...
  Object.values(mergedKitCommandMap).forEach(kitCommandConfig => {
    ...
    if (isSystemCommand(kitCommandConfig)) return

    // 允许套件禁用某个默认命令
    if (kitCommandConfig.disabled) return
