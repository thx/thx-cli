...
const { getTnpmPackage, getOutdatedPkgs, spawn, fixLength, needBlockProcessByModuleOutdated } = utils
 ...
 * MO FIXED 实现逻辑不清晰
 */
export default async function checkCliOutdated () {
  ...
  if (cliLatestVersion && semver.lt(cliLocalVersion, cliLatestVersion)) {
    outdatedPkgs.push(outdatedCliPkg = { name: pkg.name, localVersion: cliLocalVersion, latestVersion: cliLatestVersion })
  }

  // 检测依赖包版本
  const depOutdatedPkgs = await getOutdatedPkgs(CWD, NEED_CHECK_DEP_PACKAGE_LIST)
  ...
  outdatedPkgs.forEach((pkg, index) => {
    ...
    message.push(`${grey(prefix)} ${pkg.name} ${grey('本地版本：')}${pkg.localVersion}，${grey('最新版本：')}${greenBright(pkg.latestVersion)}`)
  })
  message.push(
    `\n运行 ${blueBright(`tnpm install -g ${pkg.name}`)} 或者 ${blueBright(`yarn global add ${pkg.name}`)} 即可更新。`,
    grey('CHANGELOG: http://gitlab.alibaba-inc.com/mmfs/mm-cli/blob/master/CHANGELOG.md\n') // MO TODO 缺少文档
  )
  ...
  const questions: QuestionCollection = [
    {
      ...
      name: 'isUpdate',
      default: true,
      message: `要立即更新吗（${grey('本地版本：')}${pkg.localVersion}${grey('，最新版本：')}${greenBright(pkg.latestVersion)}）？`
    },
    {
      type: 'list',
      name: 'command',
      ...
      choices: [
        ...[
          ...
          name: `${fixLength(name, 10)} ${grey(value)}`, value, short: value
        }))
