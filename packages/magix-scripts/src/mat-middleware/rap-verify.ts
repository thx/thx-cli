    ...
    this.getParsedBody = (formData, resBody) => {
      co(function*() {
        ...
        try {
          ...
          opts.log(
            `${chalk.yellow(' ⓘ 没有找到该接口在RAP上的配置:')} at ${chalk.grey(
              me.path
            )}`
          )
          return
...
function checkParams(opts, rapApiData, formData, resBody, params) {
  ...
  if (missingParams.length || redundanceParams.length) {
    ...
    opts.log(
      chalk.grey(
        `  接口详情：https://rap2.alibaba-inc.com/repository/editor?id=${rapApiData.repositoryId}&mod=${rapApiData.moduleId}&itf=${rapApiData.id}\n`
      )
    )
  }
}
