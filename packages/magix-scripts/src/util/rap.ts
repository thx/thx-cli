'use strict'
import { rap, utils } from 'thx-cli-core'
...
export default {
  ...
  getRapModels(projectId) {
    return new Promise(async (resolve, reject) => {
      ...
      if (modelJSON.relatedIds) {
        ...
        console.log(
          chalk.cyan('ⓘ 该项目RAP有关联项目，开始加载关联项目数据...')
        )
  ...
  getRap2Models(projectId) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (modelJSON.data.collaborators && modelJSON.data.collaborators.length) {
        ...
        const promises = []
        for (
          let i = 0, len = modelJSON.data.collaborators.length;
          i < len;
          i++
        ) {
  ...
  getRapModelsSingle(projectId) {
    return new Promise((resolve, reject) => {
      ...
      // 从rap上拿接口的json化数据
      const spinner = ora({
        text: `加载RAP上的接口数据，请稍候... ${chalk.gray(
          `[projectId:${projectId}]`
        )}`
      }).start()
      ...
      request(
        ...
        (error, response, body) => {
          ...
          if (!error && response.statusCode === 200) {
            ...
            resolve(modelJSON)
            spinner.succeed(
              `RAP接口数据加载完毕 ${chalk.gray(
                `[projectId:${projectId}, 耗时:${endTime - startTime}ms]`
              )}`
            )
          } else {
            reject(error)
            spinner.fail(
              `RAP接口数据加载失败 ${chalk.gray(
                `[projectId:${projectId}, 耗时:${endTime - startTime}ms]`
              )}`
            )
          }
  ...
  parseActionUrlBase(action, isRap2, supportApiPathParams) {
    ...
    if (isRap2) {
      ...
      method = utils.METHOD_MAPS[action.requestType] || 'GET'
